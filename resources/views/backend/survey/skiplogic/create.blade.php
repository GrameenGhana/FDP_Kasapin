...
@section('Title', __('labels.backend.survey.skiplogics.management') .' | '.__('labels.backend.survey.skiplogics.create'))


@section('content')

    {{ html()->form('POST', route('admin.survey.skiplogic.store'))->class('form-horizontal')->open() }}
            ...
            <div class="row">
                <div class="col-sm-5">
                    <h4 class="card-title mb-0">
                        ...
                        <small class="text-muted">@lang('labels.backend.survey.skiplogics.create')</small>
                    </h4>
                </div><!--col-->
            </div><!--row-->
                        ...
                        <div class="col-md-10">
                            <select class="form-control" name="question" id="question" required>
                                @foreach($questions as $question)
                                    <option value="{{ $question->label_c }}">{{ $question->label_c }}</option>
                                @endforeach
                            </select>
